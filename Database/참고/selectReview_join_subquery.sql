...
select g.gno, sum(quantity) as sumQuantity
from goods g join orders o
on g.gno = o.gno
where date_format(o.odate, '%y-%m') = date_format(current_date(), '%y-%m')
order by sumQuantity desc;

-- 판매량이 가장 많은 상품 2개를 조회하여 상품 번호, 상품명, 가격, 총 판매된 수량, 순위를 표시하시오.
select rank() over (order by sum(quantity) desc) as 순위, g.gno, brand, price, sum(quantity) as sumQuantity
from goods g join orders o
on g.gno = o.gno
group by g.gno
order by sumQuantity desc
...
select gno, brand, price, amount
from goods
join (select gno, sum(quantity) as amount from orders group by gno limit 0, 2) a
using (gno)
order by amount desc;

-- 이번달에 판매량이 가장 많은 상품 2개를 조회하여 상품 번호, 상품명, 가격, 총 판매된 수량, 순위를 표시하시오.
select rank() over (order by sum(quantity) desc) as 순위, g.gno, brand, price, sum(quantity) as sumQuantity
from goods g join orders o
on g.gno = o.gno
where date_format(o.odate, '%y-%m') = date_format(current_date(), '%y-%m')
...
select gno, brand, amount, price, amount * price as totalSalPrice
from goods
join (select gno, sum(quantity) as amount from orders group by gno) o
using (gno)
order by totalSalPrice desc
limit 0, 3;

-- category(분류별) 판매 수량 구하기 
-- 분류코드(cno), 분류명(name), 판매수량, 판매 금액을 판매 수량이 높은 순으로 조회하세요      
select cno, name, ifnull(amount, 0) as amount, ifnull(totalPrice, 0) as totalPrice
from category
left join (select cno, sum(quantity) as amount, sum(total) as totalPrice
from (select gno, cno, quantity, quantity * price as total from orders join goods using (gno)) a group by cno) o
using (cno)
order by amount desc;
