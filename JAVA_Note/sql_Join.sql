...
-- vender 전용
select odate, ono, o.gno, brand, id, price, quantity, price * quantity as oprice
from orders o, goods g
where o.gno = g.gno;

-- ansi 
select odate, ono, orders.gno, brand, id, price, quantity, price * quantity as oprice
from orders
join goods
using (gno);
...
select odate, ono, o.gno, brand, id, price, quantity, price * quantity as oprice
from orders o, goods g
where o.gno = g.gno and date_format(sysdate(), '%Y%m') = date_format(odate, '%Y%m');
-- where o.gno = g.gno and month(sysdate()) = month(odate) and year(sysdate()) = year(odate);

-- ansi
select odate, ono, gno, brand, id, price, quantity, price * quantity as oprice
from orders
join goods
using (gno)
where orders.gno = goods.gno and date_format(sysdate(), '%Y%m') = date_format(odate, '%Y%m');
...
-- ansi
select empno, ename, job, sal, grade, emp.deptno, dname
from emp
join salgrade
on sal between losal and hisal
join dept
...
-- vender
select ono, odate, brand, name, price, quantity, price*quantity as oprice
from orders, goods, members
where orders.gno = goods.gno and orders.id = members.id;

-- ansi
select ono, odate, brand, id, price, quantity, price*quantity as oprice
from orders
join goods
using(gno)
join members
...
-- vender 
select goods.gno, brand, count(quantity) as totalQuantity
from orders, goods
...
-- vender 이번달, 지난달 포함
select goods.gno, brand, month(odate) as thisMonth, (quantity) as totalQuantity
from orders, goods
where orders.gno = goods.gno and month(sysdate()) = month(odate)
group by gno
order by gno;
...
select gno, brand, price, cno, category.name
from goods
left join category
using(cno);
...
-- 상품 번호(gno), 상품명(brand), 총 주문된 수량을 조회
select gno, brand, ifnull(sum(quantity), 0) as totalCount
from goods
left join orders
using(gno)
group by gno;
