  ...
  methods: {
    insertTodoList() {
      if (this.todo.content.trim() != "") {
        this.$store.dispatch(Constant.ADD_TODO, {todo:this.todo}); // todo에 여기서 생성한 todo(=this.todo)를 넣어줌
        this.$router.push('/list'); // 추가가 되면 자동으로 router-view에 뿌릴 component(vue)를 바꾸도록 설정 -> $router.push('url')
        this.clear();
      }
    },
    clear() {
