...
const store = new Vuex.Store({
    ...
    actions: { // 백엔드와 통신하기 위한 메서드들
        ...
        [Constant.GET_TODO] : (store, payload) => { // 매개변수로 store 객체 자기 자신이 옴, 두번째 매개변수는 받아올 데이터
          http
            .get("/todolist/todo/" + payload.no)
            // 자신이 데이터를 갱신하지 않고 store에 commit함.
            .then(response => store.commit(Constant.GET_TODO, {todoDetail: response.data}))
            .catch(exp => alert("detail 처리에 실패하였습니다." + exp));
      },
    },
    mutations: { // 저장소에 데이터를 반영(commit시 호출됨)
        ...
        [Constant.GET_TODO] : (state, payload) =>{ // 매개변수로 store안의 state와 받아온 데이터를 줌
          state.todoDetail = payload.todoDetail // 받아온 데이터를 state에 넣어준다
      }
    }
});
